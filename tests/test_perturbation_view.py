...
def test_markdown_renders_the_expansion_with_the_gloss_headings():
    ...
    assert "first-order · refines, cannot overturn" in md
    ...
    assert md.index("Zero-order expansion") < md.index("## Execute")
...
def test_term_gloss_names_the_order():
    ...
    assert "first-order" in t.gloss and "overturn" in t.gloss
